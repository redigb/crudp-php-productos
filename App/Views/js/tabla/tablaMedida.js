...
//
//-> TABLA Tamaño
window.addEventListener('DOMContentLoaded', cargarMedida);
...
function cargarMedida() {
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            document.getElementById('tabla-medidas').innerHTML = template;
        }
    };
    xhr.send();
}
...
function eliminarMedida(id, nombre) {
    let xhr = new XMLHttpRequest();
    let url = APP_URL + 'App/ajax/medidaAjax.php';
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            xhr.send('accion=eliminarMedida&id=' + id);
        }
    });
}
