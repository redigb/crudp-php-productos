    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            categorias.forEach(function (categoria) {
                let fechaActualizada = new Date(categoria.categoria_actualizada).toLocaleDateString('es-ES', {
                    year: 'numeric',
                    month: '2-digit',
                    day: '2-digit'
                });
                ...
                template += '<tr class="border-b hover:bg-gray-100">' +
                    ...
                    '<td class="px-4 py-3">' + categoria.categoria_nombre + '</td>' +
                    '<td class="px-4 py-3 text-right">' + categoria.categoria_ubicacion + '</td>' +
                    '<td class="px-4 py-3 text-right">' + fechaActualizada + '</td>' +
                    '<td class="px-4 py-3 text-right">' +

                    `<button onclick="eliminarCategoria(${categoria.categoria_id}, '${encodeURIComponent(nombre)}')" type="button" class="text-red-700 border border-red-700 hover:bg-red-700 hover:text-white  font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:focus:ring-red-800 dark:hover:bg-red-500">
            ...
            document.getElementById('tabla-archivadores').innerHTML = template;
        }
    };
    xhr.send();
}
...
function eliminarCategoria(id, nombreCategoria) {
    let xhr = new XMLHttpRequest();
    let url = APP_URL + 'App/ajax/categoriaAjax.php';
    Swal.fire({
        title: "Quieres eliminar la categoria?",
        text: "Eliminarás la categoría '"+nombreCategoria+"' y no podrás recuperarla",
        icon: "question",
        ...
        if (result.isConfirmed) {
            ...
            xhr.send('accion=eliminarCategoria&id=' + id);
        }
    });
}
